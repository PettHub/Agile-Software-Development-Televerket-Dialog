...
export class RemoveArtist {
    ...
    private userRemoval(
        ...
        while (args.shift() && args[0]) {
            //Compiles all remaining args into a string; the reason gets put into a single string
        ...
        DatabaseFunctions.getInstance().get(querry, "art", (err, row) => {
            //Query the database to get the artist role ID (to ensure we remove the correct role)
            if (err) {
                console.log(err);
                return;
            }
            ...
            if (row) {
                ...
                if (!user.roles.cache.has(role)) {
                    ...
                    return;
                } else user.roles.remove(role); //Removes role from user
                message.reply(
                    "User: " + RemoveArtist.nick + " is not an artist anymore."
                ); //user is userID, which should be presented as username instead
                ...
                message.reply(
                    "User: " +
                        RemoveArtist.nick +
                        "'s artist role has been removed with the reason: " +
                        removeReason
                );
                //}
            }
